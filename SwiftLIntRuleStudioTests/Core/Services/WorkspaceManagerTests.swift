...
struct WorkspaceManagerTests {
    ...
    func testWorkspaceWithDeepSwiftFile() async throws {
        ...
        let swiftFile = nestedDir.appendingPathComponent("DeepFile.swift")
        try "// swift".write(to: swiftFile, atomically: true, encoding: .utf8)

        let currentWorkspace = try await withWorkspaceManager { manager in
    ...
    func testWorkspaceSkipsBuildDirectory() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("SwiftLintRuleStudioTests", isDirectory: true)
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
    ...
    func testCheckConfigFileExistsWithoutWorkspace() async throws {
        let configMissing = try await withWorkspaceManager { manager in
            manager.checkConfigFileExists()
            return manager.configFileMissing
        }
        #expect(configMissing == false)
    }
    ...
    func testWorkspaceErrorDescriptions() {
        let errors: [WorkspaceError] = [
            ...
            .invalidPath,
            .accessDenied,
            .notASwiftProject(directory: "TestDir")
        ]
        for error in errors {
