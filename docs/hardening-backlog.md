...
| **P3.x** Robustness polish | ✅ **Done** — `fetchViolations` throw (`0a19343`), selection-clear (`0bcf437`), `.merge` union (`f120008`), `analyze()` reentrancy (`ae3ec71`), fire-and-forget `Task {}` cancellation across 4 view models |
...
- **Fire-and-forget `Task {}` with no cancellation** in `GitBranchDiffViewModel`, `ConfigImportViewModel`, `MigrationAssistantViewModel`, `VersionCompatibilityViewModel` — ✅ **Done**. Each async method now stores its `Task`, cancels the prior one on re-invocation, and guards every state mutation on `!Task.isCancelled`, so a superseded run cannot overwrite the newer one. 4 deterministic regression tests (one per view model) drive "start slow run → supersede with fast run → release slow run" via a shared `SupersededTaskGate` and assert the stale result is dropped.
...
4. ~~**P3.x** robustness polish~~ ✅ — all 5 shipped (`0a19343`, `0bcf437`, `f120008`, `ae3ec71`, + fire-and-forget `Task {}` cancellation across 4 view models). Backlog cleared.
