...
struct ImpactSimulatorWorkflowTests {
    ...
    private func configureMockCLIForSafeRuleDiscovery(_ mockCLI: MockSwiftLintCLIActor) async {
        ...
        let callCounter = CallCounterActor()
    ...
    private func simulateBatchWithProgress(
        ...
        let progressCollector = ProgressCollectorActor()
        ...
        var didCollectAll = false
        while Date.now.timeIntervalSince(startTime) < 1.0 {
            let updates = await progressCollector.updates
            progressUpdates = updates
            if updates.count == ruleIds.count {
                didCollectAll = true
                break
            }
